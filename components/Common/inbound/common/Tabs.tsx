...
interface TabsComponentProps {
    ...
    tabsTrigerClassName?: string;
}

const TabsComponent = ({ tabsData, width, tabsListClassName,tabsTrigerClassName }: TabsComponentProps) => {
        ...
        <div className="container mx-auto">
            <Tabs defaultValue={tabsData[0].value}>
                ...
                <div className="flex justify-center">
                    <TabsList className={`flex  justify-center ${tabsListClassName}`} style={{ width: `${width}%` }}>
                        {tabsData.map((tab, index) => (
                            <TabsTrigger key={index} value={tab.value} className={`text-center ${tabsTrigerClassName}`}>
                        {tab.tabsTrigger}
                    </TabsTrigger>
                        ))}
                </TabsList>
